...
export async function  actualizeDataOnFirestore(currentOrgSlug,currentSubmission, currentFormId, currentSlugId, status) {
...
/*  const submissionsListRef = firestore()
  ...
  if (isSubmissionNew) {
    ...
    const document5 = await firestore().collection(`submissions_${currentOrgSlug}`).doc(currentSlugId).collection("submissionData").doc();
    const documentId=await document5.id;

    firestore().collection(`submissions_${currentOrgSlug}`).doc(currentSlugId).collection("submissionData").doc(documentId).set(fullSubmission);
return documentId;
     
  } else {

    const document5 = await firestore().collection(`submissions_${currentOrgSlug}`).doc(currentSlugId).collection("submissionData").doc(submissionId).set(fullSubmission);
...
export function fetchSubmissionData(currentOrgSlug,submissionId, slug) {

  return firestore().collection(`submissions_${currentOrgSlug}`).doc(slug)
