...
class FormDisplayForm extends React.PureComponent {
  ...
  handleNextButtonPress2 = () => {
    try {
      this.updateDataFromPage("Incomplete");
    } catch (e) {
      if (e.message === UPLOAD_ERROR) {
        Alert.alert('Please wait for the files to be uploaded.');
        return;
  ...
  render() {
    ...
    return (
      <View style={styles.fromFlowWizardContainer}>
        <Text><Text style={{fontWeight:'bold'}}>SubmissionId:</Text>
        <Text style={{color:'red'}}>{this.props.submission.submissionId}</Text>
        </Text>
        <FormProgressBar />
        ...
        <Button
            style={{width:0,height:0,opacity:0}}
              ...
              title="Next"
              onLayout={()=>{
                if(this.props.submission.submissionId==null){
                this.handleNextButtonPress2()
                }
            ...
            receiverSubmit={this.handleSubmitButtonPress}
          />
