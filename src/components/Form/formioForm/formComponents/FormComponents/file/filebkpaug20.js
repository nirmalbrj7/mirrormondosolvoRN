...
class File extends InputComponent {
  constructor(props) {
    ...
    this.onChange("ssssssssssssss", 'file');
    this.changeUploadProgress(70);
  }


  deleteUploading = async (id,docId,name) => {
...
console.log("name"+name);
    ...
    //  changeUploadProgress(90);
     ...
     .then(() => {
       ...
       let filteredArray =  this.props.data[componentkey].filter(item => item.id != id)
...
this.onChange(filteredArray, 'file');
  ...
  UploadCloudFile3 = async (file, blob, myObject) => {
    ...
    var componentkey = this.props.component.key;
    var componentData = this.props.data[componentkey];
    if (!this.props.data[componentkey]) {
      this.onChange([], 'file');
    }
    ...
    try {
      ...
      await imageRef.put(blob).then((snapshot) => {
        ...
        var mergeData = [...this.props.data[componentkey], ...newArray];
      ...
      if (url) {
        ...
        firestore()
          ...
          .then((docRef) => {
            ...
            var data = [...this.props.data[componentkey]];
          ...
          <TouchableOpacity onPress={() =>
            imageOrFile == true ?
              singleOrMultiple == true ? this.selectPhotoTappedMultiple() :
                this.selectPhotoTappedMultiple()
              :
              singleOrMultiple == true ? this.selectFileTappedMultiple() :
                this.selectFileTappedMultiple()
          }>
              ...
              componentData.map((val, index) => {
                {
                  return (<>
                    ...
                    <TouchableOpacity
                      onPress={async () => {
                        await this.deleteUploading(val.id,val.firebaseDocId,val.name)
                      }}
                    >
      ...
      <View
        ...
        {!uploading ? (
          <View>
            ...
            <Text>up{JSON.stringify(this.state.UploadingArray)}</Text>

            {
              this.state.UploadingArray && this.state.UploadingArray.length > 0 ?
                this.state.UploadingArray.map((val, index) => {
                  {
                    return (<>
                      ...
                      <TouchableOpacity
                        onPress={async () => {
                          await this.deleteUploading(val.id)
                        }}
                      >
                        <Icon name="delete" color="red" />
                      </TouchableOpacity>
