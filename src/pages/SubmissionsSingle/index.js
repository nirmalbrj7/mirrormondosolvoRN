...
const Submissions = (props) => {
    ...
    const [formDatagrid, setFormDatagrid] = useState(null);
    //submission count
    ...
    const formRef = firestore().collection(`forms_${orgSlug}`).where('slug', '==', formId);
    ...
    const fetchData = () => {
        ...
        formRef.onSnapshot(querySnapshot => {
            // console.log("qqq"+JSON.stringify(querySnapshot));
            querySnapshot.forEach(doc => {
                // console.log("qqq2"+JSON.stringify(doc));
                const { name, formEndpoint, form, datagrid } = doc.data();
                setFormName(name);
                setFormEndpoint(formEndpoint);
                setFormData(form);
                setFormDatagrid(datagrid)
            });
        });
        SubmissionRef
            .get()
            .then(querySnapshot => {
                ...
                const listSort = list.sort((a, b) => new Date(b.timestamp.seconds) - new Date(a.timestamp.seconds));


                setSubmissions(listSort);
                setIsFetching(false);
                if (isLoading) {
                    setLoading(false);
    ...
    const makeSubmissionActionButton = (submissionId) => {
        ...
        switch (routeName) {
            case 'Incomplete':
                onPressCallback = () => {
                    console.log("fomData" + formDatagrid);
                    navigation.navigate('FormView');
                    tryUpdateCurrentForm({
                        form: formData,
                        formEndpoint: formEndpoint,
                    });
                    setCurrentFormData(formName, formId, formDatagrid, formId);
            ...
            case 'Uploading':
                ...
                onPressCallback = () => {
                    ...
                    .then(() => {
                     alert('Submitted Sucessfully...please pull to refresh to see the difference');
                    });
                    ...
                    //setCurrentFormData(formName, formId, formDatagrid, formId);
                    //updateFirebaseSubmissionId(submissionId);
                    //directSubmitDataFromCloudToFormio(submissionId);
            ...
            case 'Ready':
                onPressCallback = () => {
                    console.log("fomData" + JSON.stringify(formData));
                    console.log("formEndpoint" + JSON.stringify(formEndpoint));
                    console.log("formName" + JSON.stringify(formName));
                    console.log("formDatagrid" + JSON.stringify(formDatagrid));
    ...
    if (count == 0) {
        return (
            <CardPaper style={{ margin: 30, borderRadius: 15 }}>
                ...
                />
                <Avatar.Icon size={100} icon="exception1" style={{ alignSelf: 'center', backgroundColor: 'gray', margin: 5 }} color="#fff" />
                <CardPaper.Content>

                    <Paragraph style={{ marginVertical: 5, fontWeight: 'bold', textAlign: 'center' }}>
                        No submission found with the {routeName} status
</Paragraph>
                </CardPaper.Content>
            </CardPaper>
        );
